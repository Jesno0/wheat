...
cls.prototype.getResourceList = async function (catalogues,save_path,is_cache) {
    ...
    for(let name of catalogues.values()) {
        ...
        const result = await circle(type_info,save_path,is_cache);
        ...
        switch (type) {
            ...
            case Bornforlove.types.picture_list:
                ...
                for(let url of urls.values()) {
                    const infos = await Bornforlove[type](url,is_cache);
            ...
            case Bornforlove.types.picture_detail:
                for(let url of urls.values()) {
                    const res_infos = await Bornforlove[type](url,is_cache);
